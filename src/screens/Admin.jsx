...
export const Admin = () => {
    ...
    const handleAprobar = (idSolicitud) => {
        ...
        setSelectedSolicitud(solicitudesMostradas.find(solicitud => solicitud.id_solicitud === idSolicitud));
    };
                ...
                <tbody>
                    {solicitudesMostradas.map(solicitud => (
                        <tr key={solicitud.id_solicitud}>
                            ...
                            <td>
                                <button className="ver-mas-btn" onClick={() => openModal(solicitud)}>
                                    Ver más
                                </button>
                            </td>
                        </tr>
            ...
            {isModalOpen && selectedSolicitud && (
                <div className="modal">
                    <div className="modal-content">
                        ...
                        <p className="detail-info">Descripción: {selectedSolicitud.descripcion}</p>
                        <p className="detail-info">Tipo de Organización: {selectedSolicitud.tipo_organizacion}</p>
                        <p className="detail-info">Dirección: {selectedSolicitud.direccion}</p>
                        <p className="detail-info">País: {selectedSolicitud.pais}</p>
                        <p className="detail-info">Departamento: {selectedSolicitud.departamento}</p>
                        <p className="detail-info">Ciudad: {selectedSolicitud.ciudad}</p>
                        <p className="detail-info">Teléfono: {selectedSolicitud.telefono}</p>
                        <p className="detail-info">Email: {selectedSolicitud.email}</p>
                        <p className="detail-info">Web: {selectedSolicitud.web}</p>
                        <p className="detail-info">Nombre de Contacto: {selectedSolicitud.nombre_contacto}</p>
                        <p className="detail-info">Email de Contacto: {selectedSolicitud.email_contacto}</p>
                        {showCoordsInput && (
                        ...
                        {selectedSolicitud.estado === 'rechazado' ? (
                            <div className="actions-buttons">
                                {!showCoordsInput ? (
                                    <button className="aprobar-button" onClick={() => handleAprobar(selectedSolicitud.id_solicitud)}>
                                        Aprobar
                                    </button>
                                ) : (
                                ...
                                <div className="actions-buttons">
                                    {!showCoordsInput ? (
                                        <>
                                            ...
                                            <button className="rechazar-button" onClick={() => handleRechazar(selectedSolicitud.id_solicitud)}>
                                                Rechazar
                                            </button>
                                        </>
                                    ) : (
